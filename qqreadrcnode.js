...
let scheduleCronstyle = () => {
  ...
  schedule.scheduleJob('0 */30 * * * *', () => {
    ...
    tz = ""; 
  //随机等待1-10分钟执行
      setTimeout(all,(Math.floor(Math.random() * 10) + 1) * 1000 * 60);
    }
  });
}
...
async function all() {
  ...
  for (let i = 0; i < QQ_READ_COOKIES.qqreadbodyVal.length; i++) {
    ...
    if (!cookie_is_live) {
      ...
      QQ_READ_COOKIES.qqreadtimeurlVal.splice(i, 1);
      QQ_READ_COOKIES.qqreadtimeheaderVal.splice(i, 1);
      i--;
      continue;
    }
    if (BOX == 0) {
  ...
  return new Promise((resolve, reject) => {
    ...
    $.get(toqqreadinfourl, (error, response, data) => {
      ...
      if (!info.data.user) {
        ...
        let cookie_not_live_message = new Date(new Date().getTime()).toLocaleString() + "账号" + account_count + "COOKIE失效";
